/**
 * WhatsApp Engine Abstraction Layer — Migrated from apps/dashboard/lib/whatsapp-engine.ts
 * Standalone version for the wa-forge microservice (no Nuxt $fetch dependency)
 *
 * Supports: WuzAPI, GoWA, OpenWA, Hybrid (multi-engine load balancer)
 */
...
/** Unified fetch wrapper (uses native fetch instead of Nuxt $fetch) */
async function apiCall(
  ...
  let finalPath = path
  if (targetEngine === 'wuzapi' || targetEngine === 'gowa') headers['token'] = token
  ...
  if (targetEngine === 'gowa') {
    ...
    if (!finalPath.startsWith('/sessions/')) finalPath = `/sessions/${token}${finalPath}`
  }
  ...
  try {
    ...
    if (!res.ok) {
      // Auto-register device on 401/404 (same retry logic from original)
      if (res.status === 401 && targetEngine === 'wuzapi' && retry) {
        try {
          await fetch(`${cfg.base}/admin/users`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': process.env.WUZAPI_TOKEN || '' },
      ...
      if (targetEngine === 'gowa' && data?.code === 'DEVICE_NOT_FOUND' && retry) {
        try {
          await fetch(`${cfg.base}/devices`, { method: 'POST', headers, body: JSON.stringify({ device_id: token }) })
          return apiCall(path, token, method, body, false, targetEngine)
        } catch { /* fall through */ }
      }
      if (data?.code !== 'DEVICE_NOT_FOUND') {
        throw new Error(`[${targetEngine}] ${res.status}: ${JSON.stringify(data)}`)
      }
      return data
    }
    return data
...
async function getEngineStatusSingle(token: string, target: 'wuzapi' | 'gowa' | 'openwa'): Promise<EngineStatus> {
  try {
    ...
    if (target === 'wuzapi') {
      const isConnected = Boolean(data?.data?.Connected || data?.data?.connected || data?.Connected || data?.connected || data?.data?.LoggedIn)
      const isLoggedIn = Boolean(data?.data?.LoggedIn || data?.data?.logged_in || data?.LoggedIn || isConnected)
      return { connected: isConnected, loggedIn: isLoggedIn, phone: data?.data?.Jid?.split('@')[0] || data?.Jid?.split('@')[0] || undefined, engine: 'WuzAPI' }
    } else if (target === 'gowa') {
      const isConnected = Boolean(data?.results?.is_connected || data?.results?.connected || data?.Connected)
      const isLoggedIn = Boolean(data?.results?.is_logged_in || data?.results?.logged_in || isConnected)
      return { connected: isConnected, loggedIn: isLoggedIn, phone: data?.results?.jid?.split('@')[0] || data?.results?.phone?.split('@')[0] || undefined, engine: 'go-whatsapp-web-multidevice' }
    } else {
      const isConnected = data?.status === 'CONNECTED' || data?.status === 'AUTHENTICATED' || data?.connected === true
      const isLoggedIn = isConnected || data?.loggedIn === true
      return { connected: isConnected, loggedIn: isLoggedIn, phone: data?.phone?.split('@')[0] ?? data?.id?.split('@')[0] ?? undefined, engine: 'OpenWA' }
    }
  } catch {
...
async function getQRCodeSingle(token: string, target: 'wuzapi' | 'gowa' | 'openwa'): Promise<string | null> {
  try {
    ...
    if (target === 'wuzapi') {
      ...
      if (qr && !qr.startsWith('data:image')) return `data:image/png;base64,${qr}`
      return qr
    } else if (target === 'gowa') {
      const qrLink = data.results?.qr_link ?? data.data?.qr_link ?? data.qr_link
      if (qrLink) {
        const fetchUrl = qrLink.replace('localhost:3000', 'gowa:3000').replace('localhost:3200', 'gowa:3000').replace('127.0.0.1:3000', 'gowa:3000')
        const gowaToken = process.env.GOWA_TOKEN || 'secret-token'
        const imgRes = await fetch(fetchUrl, { headers: { 'Authorization': `Basic ${Buffer.from(`admin:${gowaToken}`).toString('base64')}` } })
...
async function sendPresenceSingle(token: string, phone: string, messageLength: number, target: 'wuzapi' | 'gowa' | 'openwa'): Promise<void> {
  try {
    ...
    if (target === 'wuzapi') {
      await apiCall('/chat/presence', token, 'POST', { Phone: `${cleanPhone}@s.whatsapp.net`, State: 'composing' }, true, target)
    } else if (target === 'gowa') {
      await apiCall('/send/presence', token, 'POST', { phone: cleanPhone, state: 'composing' }, true, target)
    } else {
      await apiCall('/presence', token, 'POST', { chatId: `${cleanPhone}@c.us`, presence: 'composing' }, true, target)
    }
...
async function checkPhoneSingle(token: string, phone: string, target: 'wuzapi' | 'gowa' | 'openwa'): Promise<boolean> {
  try {
    ...
    if (target === 'wuzapi') {
      const data = await apiCall('/chat/check/phone', token, 'POST', { Phone: `${cleanPhone}@s.whatsapp.net` }, true, target)
      ...
      const data = await apiCall('/check-number', token, 'POST', { phone: cleanPhone }, true, target)
...
export async function getEngineStatus(token: string): Promise<EngineStatus> {
  if (ENGINE === 'hybrid') {
    ...
    const statuses = results.filter((r): r is PromiseFulfilledResult<EngineStatus> => r.status === 'fulfilled').map(r => r.value)
    ...
    return {
      connected: anyConnected, loggedIn: anyLoggedIn, phone,
      engine: activeEngines.length > 0 ? `Hybrid (${activeEngines.join(' + ')})` : 'Hybrid Multi-Engine (WuzAPI + GoWA + OpenWA)',
    }
  }
...
export async function sendMessage(token: string, phone: string, message: string): Promise<SendResult> {
  let cleanPhone = phone.replace(/[\+\s\-]/g, '')
  if (cleanPhone.startsWith('00')) cleanPhone = cleanPhone.substring(2)
  if (cleanPhone.length === 10 && cleanPhone.startsWith('3')) cleanPhone = `39${cleanPhone}`
  ...
  if (ENGINE === 'hybrid') {
    ...
    for (let i = 0; i < ACTIVE_ENGINES.length; i++) {
      ...
      try {
        ...
        let endpoint = ''
        if (target === 'wuzapi') { endpoint = ENDPOINTS.wuzapi.sendText; body = { Phone: `${cleanPhone}@s.whatsapp.net`, Body: message } }
        else if (target === 'gowa') { endpoint = ENDPOINTS.gowa.sendText; body = { phone: cleanPhone, message } }
        else { endpoint = ENDPOINTS.openwa.sendText; body = { chatId: `${cleanPhone}@c.us`, text: message } }
      ...
      } catch (err: any) { errors.push(`[${target}]: ${err.message || err}`) }
    }
...
function resolveFullMediaUrl(url: string): string {
  ...
  const baseUrl = process.env.APP_URL || 'http://host.docker.internal:3000'
  return `${baseUrl.replace(/\/$/, '')}/${url.replace(/^\//, '')}`
}

export async function sendMedia(token: string, phone: string, mediaType: 'image' | 'video' | 'audio' | 'document', fileUrl: string, caption?: string): Promise<SendResult> {
  ...
  if (ENGINE === 'hybrid') {
    ...
    for (let i = 0; i < ACTIVE_ENGINES.length; i++) {
      ...
      try {
        ...
        if (target === 'wuzapi') { endpoint = `/chat/send/${mediaType}`; body = { Phone: `${cleanPhone}@s.whatsapp.net`, Url: fullFileUrl, Caption: caption || '' } }
        else if (target === 'gowa') { endpoint = `/send/${mediaType}`; body = { phone: cleanPhone, [mediaType]: fullFileUrl, caption: caption || '' } }
        else { endpoint = `/messages/send-${mediaType === 'document' ? 'file' : mediaType}`; body = { chatId: `${cleanPhone}@c.us`, url: fullFileUrl, caption: caption || '' } }
        const data = await apiCall(endpoint, token, 'POST', body, true, target)
...
export async function disconnectEngine(token: string): Promise<void> {
  if (ENGINE === 'hybrid') {
    await Promise.allSettled(ACTIVE_ENGINES.map(async target => {
      if (target === 'gowa') await apiCall(`/devices/${token}`, token, 'DELETE', undefined, true, target)
      else await apiCall(ENDPOINTS[target].logout, token, 'POST', undefined, true, target)
    }))
    return
  }
  if (ENGINE === 'gowa') await apiCall(`/devices/${token}`, token, 'DELETE')
  else await apiCall(ENDPOINTS[ENGINE as any].logout, token, 'POST')
}
