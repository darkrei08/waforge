...
Restituisci ESCLUSIVAMENTE il testo migliorato e formattato, pronto per essere copiato. NESSUN commento introduttivo, NESSUNA spiegazione, NESSUN meta-testo come "Ecco il testo migliorato:" o simili. Il primo carattere della tua risposta DEVE essere il primo carattere del messaggio finale.`
  ...
  // Aggiunta globale delle regole di formattazione ufficiali per WhatsApp
  systemPrompt += `\n\nREGOLE DI FORMATTAZIONE WHATSAPP UFFICIALI (ASSOLUTAMENTE OBBLIGATORIE):
- Grassetto: usa *testo*
- Corsivo: usa _testo_
- Barrato: usa ~testo~
- Spaziatura fissa (Monospace): usa \`\`\`testo\`\`\`
- Elenchi puntati: usa ESCLUSIVAMENTE il trattino e spazio (- testo). NON USARE MAI L'ASTERISCO (*) per gli elenchi per evitare conflitti col grassetto.
- Elenchi numerati: usa numero, punto e spazio (es. 1. testo)
- Citazione: usa simbolo maggiore e spazio (> testo)
- Codice in linea: usa apice singolo (\`testo\`)
Usa SOLO queste formattazioni. Evita qualsiasi altro markup Markdown o HTML non supportato da WhatsApp.`
  ...
  try {
    ...
    for (const cmd of mcpServers) {
      ...
      // Auto-fix per Cockpit Tools MCP: evita 'npx -y cockpit-tools-mcp' (E404) e usa il file locale
      let finalCmd = cmd;
      if (finalCmd.includes('npx') && finalCmd.includes('cockpit-tools-mcp')) {
        finalCmd = 'node ./bin/cockpit-tools-mcp.mjs';
      }

      const args = finalCmd.split(' ')
