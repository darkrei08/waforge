          ...
          <div class="flex-1 flex overflow-hidden relative">
            ...
            <div v-if="showAiPanel" class="w-[450px] xl:w-[490px] flex-shrink-0 flex flex-col bg-[#0e1014]/95 border-l border-white/10 overflow-hidden transition-all duration-300 animate-fade-in">
              ...
              <div class="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
                ...
                <!-- Chat History Feed -->
                <AiMessageBubble 
                  v-for="(msg, i) in chatHistory" :key="i"
                  :role="msg.role as any"
                  :content="msg.content"
                >
                  <template #actions v-if="msg.role === 'assistant' && isAiReadyContent(msg.content, i)">
                    <button @click="applyToBody(msg.content)" 
                            class="text-xs bg-primary text-white font-bold px-3 py-1.5 rounded-lg hover:bg-primary/90 transition-all flex items-center gap-1.5 shadow-sm">
              ...
              <div class="p-4 bg-[#111318] border-t border-white/10 shrink-0">
                <ChatInputCapsule 
                  ...
                  @send="handleAiGenerate('chat')" 
                />
                <div v-if="chatHistory.length > 0" class="flex justify-center mt-2">
                  <button @click="chatHistory = []" type="button" class="text-[10px] text-gray-500 hover:text-red-400 transition-colors">🗑️ Pulisci chat</button>
                </div>
              </div>
